...
//#define _SETTINGS_LOAD_DEBUG // отладочный режим загрузки настроек из EEPROM (выводит ошибки чрения заголовков записей)
#define DEBUG_SERIAL Serial // какой Serial использовать для дебаг-режима


//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// тип используемой EEPROM
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#define EEPROM_CLASS AT24C128 // поддерживаемые типы: AT24C128, AT24C256, AT24C512, AT241024
...
/*
#define CURRENT_OSCILL_FREQ               2000 // через сколько микросекунд собирать значение тока с АЦП, для помещения в список
#define COUNT_OF_FULL_CURRENT_LIST        30  // сколько записей должно быть в списке по току, не считая записей в списке предпросмотра
#define COUNT_OF_CURRENT_PREVIEW_RECORDS  20 // кол-во записей в списке предпросмотра перед превышением по току
*/
...
#define MAX_PULSES_TO_CATCH       3000 // сколько импульсов ловим на энкодере максимально?
...
#define EEPROM_LAST_3_RECORD_SIZE                     30000ul // максимальный размер одной записи срабатывания, для хранения в EEPROM (примерно 30 килобайт на одну запись)
//--------------------------------------------------------------------------------------------------------------------------------
