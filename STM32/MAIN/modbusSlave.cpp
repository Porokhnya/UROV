...
//#define  ledPin13  13       // Ќазначение светодиодов на плате
#define T35  5
...
void modbusSlave::setRxTxSwitch(uint8_t pin)
{
	rxtxSwitch = pin;
	
	if(rxtxSwitch != 0xFF)
	{
		pinMode(rxtxSwitch, OUTPUT);
		switchToReceive();
	}
}

void modbusSlave::switchToSend() // переводим контроллер RS-485 на передачу
{
   if(rxtxSwitch != 0xFF)
   {
	   digitalWrite(rxtxSwitch,HIGH); 
   }
}

void modbusSlave::switchToReceive() // переводим контроллер RS-485 на приём
{    
  if(rxtxSwitch != 0xFF)
  {
	  digitalWrite(rxtxSwitch,LOW); 
  }
}
...
void modbusSlave::setSerial(Stream* s)
{	
	port = s;	 	 
	u8lastRec = 0;
...
void modbusSlave::getDigitalStatus(byte funcType, word startreg, word numregs)
{
	...
	//для каждой группы из 8 регистров длины сообщения увеличивается на 1
	_len = numregs/8;
	//if there is at least one incomplete byte's worth of data
	//Если там стоит по крайней мере одного неполного байта данных
	//then add 1 to the message length for the partial byte.
	//затем добавить 1 к длине сообщения для частичного байта.
	if(numregs%8)
	{
		_len++;
	}
	//allow room for the Device ID byte, Function type byte, data byte count byte, and crc word
	//чтобы места для байта идентификатор устройства, байт Тип функции, количество байт данных, байт, а слово CRC
	_len +=5;
	...
	//выделяет память соответствующего размера для сообщения
	_msg = (byte *) malloc(_len);
	...
	//По количеству регистров запросы
	while(numregs--)
	{
...
void modbusSlave::getInputRegister(byte funcType, word startreg, word numregs)
{
	word val;
	word i = 0;
...
void modbusSlave::setMULTIPLE_Status(byte funcType, word reg, word val)
{
  ...
  // read each coil from the register map and put its value inside the outcoming message  
  // читать каждый бит от регистра  и положить его значение внутри исходящего сообщения
...
int modbusSlave::run(void)
{
	...
	// задача - принять каждый байт из буфера, дождаться окончания кадра, и потом - анализировать уже
	if(port->available())
	{
		uint8_t b = port->read();
	...
	if ((micros() - u32time) < T3_5)
  {
	 // port->println("((micros() - u32time) < T3_5)");
	  return 0;            // Если интервал меньше заданного - возврат
  }
  ...
  if (i8state < 7) 
  {
	//  port->println("i8state < 7");
	//  port->print("i8state = ");
	//  port->println(i8state);
	  
	  _len = 0; // обнуляем
	  free(_msg);
	  _msg = NULL;
	  
	  return i8state;  //  return i8state;    
  }
...
/*	
	...
	// ОШИБКА В STM32GENERIC, AVAILABLE возвращает не кол-во байт в приемном буфере, а всегда 1 !!!
	// ПОЭТОМУ КОД ДАННОЙ ФУНКЦИИ РАБОТАЕТ НЕПРАВИЛЬНО !!!
	_len = port->available();
	...
	if(_len == 0)
	 ...
	 // проверить Т35 после завершения кадра или еще  нет окончания кадра
  if (_len != u8lastRec) 
	  {
		u8lastRec = _len;
		//u32time = millis();
  ...
  //if ((unsigned long)(millis() - u32time) < (unsigned long)T3_5)
  if ((micros() - u32time) < T3_5)
  {
	  port->println("((micros() - u32time) < T3_5)");
	  return 0;            // Если интервал меньше заданного - возврат
...
*/
	...
	//for(uint8_t i = 0; i<_len;i++)
	//{
	//	port->print(_msg[i], HEX);port->print(" ");
	//}
	//port->println();
	...
	if( (_msg[0] != 0xFF) && (_msg[0] != _device->getId()) )
		{
		//	port->println("if( (_msg[0] != 0xFF) && (_msg[0] != _device->getId()) )");
	...
	uint16_t crcPassed = _msg[_len - 2];
	crcPassed <<= 8;
	crcPassed |= _msg[_len - 1];

	//if the checksum does not match, ignore the message
	if ( _crc != crcPassed)
	{
	//	port->println("BAD CRC!!!!");
	//	port->print("crc computed: "); port->println(_crc, HEX);
	//	port->print("crc passed: "); port->println(crcPassed, HEX);
	...
	field1	= ((uint16_t)(_msg[2]) << 8) | _msg[3];

	//copy field 2 from the incoming query
	field2  = ((uint16_t)(_msg[4]) << 8) | _msg[5];
	...
	switch(funcType)
		{
			...
			case READ_INPUT_REGISTER:
				this->getInputRegister(funcType, field1, field2);
				break;
			case READ_REGISTERS:
				this->getInputRegister(funcType, field1, field2);
				break;
