...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void SDFakeFilesClass::addToLS(const String& requestedDirName, Stream* pStream) // добавляем наш фейковый файл в список LS
{
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool SDFakeFilesClass::isOurFakeFile(const String& fileName) // проверяем, не запросили ли наш фейковый файл
{
  return fileName.endsWith(OUR_LAST_3_FAKE_FILE_NAME);
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
uint32_t SDFakeFilesClass::getFileSize(const String& fileName) // возвращаем размер нашего фейкового файла
{
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void SDFakeFilesClass::printFakeFile(const String& fileName, Stream* pStream) // печатаем наш фейковый файл в поток
{
  if(fileName.endsWith(OUR_LAST_3_FAKE_FILE_NAME)) // это файл с последними тремя срабатываниями
  {
