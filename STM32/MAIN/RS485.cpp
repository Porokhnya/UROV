...
//--------------------------------------------------------------------------------------------------
void RS485::begin() // начинаем работу
{
  DBGLN(F("RS485: begin."));
...
//--------------------------------------------------------------------------------------------------
void RS485::switchToSend() // переводим контроллер RS-485 на передачу
{
   digitalWrite(dePin,HIGH); 
}
//--------------------------------------------------------------------------------------------------
void RS485::switchToReceive() // переводим контроллер RS-485 на приём
{    
  digitalWrite(dePin,LOW); 
}
//--------------------------------------------------------------------------------------------------
void RS485::send(RS485PacketType packetType, const uint8_t* data, uint16_t dataLength) // посылаем пакет с данными
{
...
//--------------------------------------------------------------------------------------------------
void RS485::waitTransmitComplete() // ждём окончания передачи
{
  workStream->flush(); 
}
//--------------------------------------------------------------------------------------------------
RS485Packet RS485::getDataReceived(uint8_t* &data) // возвращаем принятый пакет
{
    data = dataReceived;
    return rs485Packet;
}
//--------------------------------------------------------------------------------------------------
void RS485::clearReceivedData() // очищаем принятый пакет
{
	delete[] dataReceived;
	dataReceived = NULL;
	rsPacketPtr = (uint8_t*)&rs485Packet;
...
//--------------------------------------------------------------------------------------------------
void RS485::update() // обновляем внутреннее состояние
{
  switchToReceive();  
...
//--------------------------------------------------------------------------------------------------
bool RS485::gotRS485Packet() // проверяем, есть ли у нас полный RS-485 пакет
{  
...
//--------------------------------------------------------------------------------------------------
bool RS485::processRS485Packet() // обрабатываем пакет
{
...
//--------------------------------------------------------------------------------------------------
uint8_t RS485::crc8(const uint8_t *addr, uint16_t len) // подсчёт контрольной суммы
{
  uint8_t crc = 0;
