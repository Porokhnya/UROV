...
void CreateEncoderChartScreen::create_Schedule(TFTMenu* menu)  //  Сформировать график
{
  ...
  if(chartPoints.size())
  {
   ...
   //  посчитали кол-во точек по частям, выводим это в Serial
    ...
    // бежим по массиву точек, берём точку, берём следующую точку. Время до между текущей и следующей точкой - ширина импульса. Относительный размах точки по Y - влияет на ширину импульса, чем координата Y меньше, тем импульс - короче.
